...
def generate_cover_letter(state: CoverLetterGraphState) -> StateGraph:
    ...
    corrections_client = CorrectionsClient()
    sentences_to_avoid = corrections_client.fetch_corrections("sentence")
    words_to_avoid = corrections_client.fetch_corrections("word")
    state["sentences_to_avoid"] = [sentence["text"] for sentence in sentences_to_avoid]
    state["words_to_avoid"] = [word["text"] for word in words_to_avoid]    

    result = agent.agent_generate_cover_letter(state)
