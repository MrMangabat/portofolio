# backend/services/service_cover_letter/src/api/routes/routes_files.py
import logging
...
from src.data_models.object_storage.minio.minio_models import FileItem
from src.data_models.events.kafka.producers.file_upload_event import FileUploadedEvent
from src.service_layer.file_service import FileService
from src.data_repositories.minio_repository import MinioRepository
from src.data_repositories.qdrant_repository import QdrantCoverLetterRepository
from src.event_broker.producers.file_uploaded_producer import FileUploadedProducer
from io import BytesIO
import uuid
from src.service_layer.embedding_file_service import FileEmbeddingService
    ...
    try:
        ...
        for file_item, raw_file in zip(minio_responses, files):
            ...
            if not extracted_text:
                logging.warning(f"{__file__} | No text extracted for {file_item.original_file_name}")
                continue
...
def get_files(bucket: str, file_service: FileService = Depends(get_file_service)) -> List[FileItem]:
    logging.info(f"{__file__} | Received GET for bucket={bucket}")
    try:
